...
**Available Types:**
* `"pollinations"`: Standard URL-based endpoint. Does not require an API key.
  * *Required fields:* `id`, `type` (`"pollinations"`), `baseUrl`, `model`.
* `"gemini-native"`: Used for Google Gemini Imagen endpoints (`generateContent`).
  * *Required fields:* `id`, `type` (`"gemini-native"`), `baseUrl`, `apiKeyEnv`.
...
* `"openai"`: Used for standard OpenAI image generation endpoints (`/v1/images/generations`).
