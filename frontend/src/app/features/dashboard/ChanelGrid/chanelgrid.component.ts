...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TelegramIconComponent, XIconComponent],
  template: `
    <div class="grid grid-cols-2 border border-border rounded-3xl overflow-hidden bg-transparent">
      @for (channel of channels(); track channel.id; let i = $index) {
        <div class="py-4 px-8 flex items-center justify-center transition-colors hover:bg-white/[0.03]"
             ...
             [class.border-b]="i < (channels().length % 2 === 0 ? channels().length - 2 : channels().length - 1)">
          <div class="flex items-center gap-4 text-white w-full max-w-lg">
            ...
            <div class="flex flex-col gap-1">
              ...
              <span class="text-[10px] px-2 py-0.5 rounded w-fit inline-block font-semibold mt-1"
